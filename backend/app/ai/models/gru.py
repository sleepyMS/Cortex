...
class GRUClassifier(BaseAIModel):
            ...
            if has_validation:
                ...
                with torch.no_grad():
                    ...
                    if is_regression and epoch == 0:
                        logger.info(f"[DEBUG] GRU Regression val_outputs shape: {val_outputs.shape}, y_val_t shape: {y_val_t.shape}")
    ...
    def to_onnx(self, path: str, sample_input_shape: Optional[Tuple[int, ...]] = None) -> None:
        ...
        # 샘플 입력 생성 - batch_size=1로 고정 (GRU 호환성)
        if sample_input_shape is None:
            sample_input_shape = (1, 60, self.config.input_size)
        else:
            # batch_size를 항상 1로 강제 (GRU hidden state 경고 방지)
        ...
        # GRU의 경우 batch_size를 동적으로 하면 hidden state 관련 경고 발생
