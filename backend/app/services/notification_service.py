...
class NotificationService:
    ...
    async def send_optimization_completed_notification(self, db_session, job_id: str):
        """
        최적화 완료 알림을 email_service를 통해 보냅니다.
        """
        ...
        if not job or not job.user or not job.strategy:
            logger.warning(f"Notification service: Optimization Job {job_id} or relations not found. Aborting email.")
            return
        ...
        # 3. email_service를 통해 템플릿 생성
        email_content = self.email_service.get_optimization_completed_content(context)
