...
개인 투자자가 자신만의 투자 전략을 데이터 기반으로 검증(백테스팅)하고, **AI/ML 모델을 활용한 시장 예측**을 전략에 통합하며, 커뮤니티와 공유하고, 최종적으로 자동매매까지 연결할 수 있는 **사용자 참여형 올인원 퀀트 플랫폼**을 개발한다.

## 3. 핵심 컨셉 (Core Concepts)

- **데이터 기반 의사결정:** 모든 투자 판단의 근거를 데이터로 제공하여 감정적인 매매를 배제한다.
- **AI 기반 시장 예측:** LSTM, GRU, TFT 등 딥러닝 모델을 활용하여 시장 방향성을 예측하고, 이를 전략의 신호로 활용한다.
- **사용자 경험 최우선:** 복잡한 퀀트 개념과 전략 생성을 누구나 쉽게 사용할 수 있도록 직관적인 UI/UX로 설계한다.
- **자동화를 통한 자유:** 검증된 전략을 24시간 자동으로 실행하여 사용자의 시간과 감정 소모를 줄인다.
- **커뮤니티를 통한 성장:** 다른 사용자와 전략 및 AI 모델을 공유하며 함께 성장하는 생태계를 구축한다.
...
- **MLOps 파이프라인:** 재현 가능한 AI 모델 학습, 버전 관리, Walk-Forward Retraining을 통한 지속적인 모델 개선을 지원한다.
